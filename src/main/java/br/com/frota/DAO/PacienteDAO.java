...
public class PacienteDAO extends ConexaoDB{
    ...
    public Paciente insert(Paciente entidade) {
        try (PreparedStatement preparedStatement = prepararSQL(INSERT_PACIENTE_SQL, java.sql.Statement.RETURN_GENERATED_KEYS)) {

            preparedStatement.setString(1, entidade.getNome());
            preparedStatement.setTimestamp(2, entidade.getDtNascimento());
    ...
    public Paciente findById(Integer id) {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(SELECT_PACIENTE_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                Timestamp dtNascimento = rs.getTimestamp("dt_nascimento");
    ...
    public List<Paciente> selectAllPacientes() {
        ...
        try (PreparedStatement preparedStatement = prepararSQL(SELECT_ALL_PACIENTE)) {
            ...
            while (rs.next()) {
                ...
                String nome = rs.getString("nome");
                Timestamp dtNascimento = rs.getTimestamp("dt_nascimento");
                entidades.add(new Paciente(id, nome, dtNascimento));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
